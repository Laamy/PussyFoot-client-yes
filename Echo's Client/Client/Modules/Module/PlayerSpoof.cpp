...
void PlayerSpoof::onRender(class Renderer* renderer) {
	...
	for (auto ent : entities) {
		if (ent == player || ent->getType() != "player") continue;
		if (offset > 16) break;
		std::string text(ent->getUsername() + " | " +
			std::to_string((int)ent->getPos()->x) + ", " + std::to_string((int)ent->getPos()->y) + ", " + std::to_string((int)ent->getPos()->z));
